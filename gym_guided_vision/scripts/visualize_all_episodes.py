...
def main(args):
    ...
    # sort episode files by episode number "episode_i.hdf5"
    episode_files = sorted(episode_files, key=lambda x: int(re.search(r'episode_(\d+)', x).group(1)))
    ...
    for i, episode_file in enumerate(episode_files):
        ...
        if f'episode_{i}' not in episode_file:
